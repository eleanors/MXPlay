...
(function(window, undefined) {
        ...
        //不能这样做，因为几个应用程序，包括ASP.NET跟踪
        //通过arguments.caller.callee和Firefox堆栈如果死亡
        //你试图跟踪通过"使用严格的"调用链。 （＃13335）
        //技术支持：Firefox的18+
        //"使用严格";
        var
        //递延使用DOM的准备
        ...
        //技术支持：IE<10
        //对于`typeof运算xmlNode.method而不是`xmlNode.method！==未定义`
        ...
        // 一个简单的方法来检查HTML字符串
        //优先# ID在<标签>避免XSS通过location.hash（# 9521）
        //严格的HTML /识别（# 11290：必须从<）
        rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/,
        ...
        // 使用jQuery.camelCase回调 to replace()
        ...
        completed = function(event) {
                ...
                // readyState === "complete" is good enough(不够好) for us to call the dom ready in oldIE
        ...
        jQuery.fn = jQuery.prototype = {
                                ...
                                if (match && (match[1] || !context)) {
                                        ...
                                        if (match[1]) {
                                                ...
                                                if (elem && elem.parentNode) {
                                                        // Handle the case(处理这种情况) where IE and Opera return items
                                                        //按名称而不是ID
                                                        if (elem.id !== match[2]) {
                                                                return rootjQuery.find(selector);
                                                        }

                                                        // 否则，我们的元素直接注入到jQuery对象
                                                        this.length = 1;
                                                        this[0] = elem;
                                                }
                                        ...
                                        // HANDLE: $(expr, context)
                                        // (which is just equivalent to(这仅仅是等效): $(context).find(expr)
                                } else {
                ...
                //仅供内部使用。
                //就像一个数组的方法，而不是像一个jQuery的方法。
                push: core_push,
                sort: [].sort,
                splice: [].splice
        };

        // 最后给init函数原型上添加jQuery的原型实例
        ...
        jQuery.extend = jQuery.fn.extend = function() {
                ...
                // 处理案件时的目标是一个字符串或某事（可能在深层副本）
                if (typeof target !== "object" && !jQuery.isFunction(target)) {
                        target = {};
                }

                // 如果只有一个参数被传递,则是扩展jQuery的本身
                if (length === i) {
                        target = this; //通过调用的方式我们就能确实当前的this的指向，所以这时候就能确定target了。最后就很简单了，通过for循环遍历把数据附加到这个target上了。当然在这个附加的过程中我们还可以做数据过滤、深拷贝等一系列的操作了。
						--i;
                }
        ...
        jQuery.ready.promise = function(obj) {
                if (!readyList) {
                        ...
                        if (document.readyState === "complete") {
                                ...
                                // A fallback to window.onload, that will always work回退到window.onload，将始终工作
                                ...
                                if (top && top.doScroll) { (function doScrollCheck() {
                                                if (!jQuery.isReady) {
                                                        ...
                                                        // detach all dom ready events   卸下所有的DOM ready事件
                                                        detach();

                                                        // and execute any waiting functions 并执行任何等待功能
                                                        jQuery.ready();
                                                }
                                        })();
                                }
        ...
        // Populate(填充) the class2type map
 ...
 */
        jQuery.Callbacks = function(options) {
                ...
                self = {
                        ...
                        // 将元素移出DOM，当我们想将元素自身移除时我们用 .remove()，同时也会移除元素内部的一切，包括绑定的事件及与该元素相关的jQuery数据
                        remove: function() {
                                if (list) {
        ...
        jQuery.extend({
                ...
                /* , ..., subordinateN */
                        ...
                        // the count of uncompleted subordinates   未完成下属的计数
                        remaining = length !== 1 || (subordinate && jQuery.isFunction(subordinate.promise)) ? length: 0,

                        // the master Deferred. If resolveValues consist of only a single Deferred, just use that.主人递延。如果resolveValues只包含一个延期,就使用它。
                        deferred = remaining === 1 ? subordinate: jQuery.Deferred(),

                        // Update function for both resolve and progress values 更新值函数解决和进展
        ...
        jQuery.fn.extend({
                ...
                //  对于该方法而言，该对象指向已经从 DOM 中被移除的对象，而不是指向替换用的对象。
                ...
                //  分离
                //  如果你想删除元素，不破坏他们的数据或事件处理程序（这些绑定的信息还可以在之后被重新添加回来）
